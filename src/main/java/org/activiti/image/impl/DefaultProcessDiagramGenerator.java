...
public class DefaultProcessDiagramGenerator implements ProcessDiagramGenerator {
  ...
  public DefaultProcessDiagramGenerator(final double scaleFactor) {
    ...
    activityDrawInstructions.put(BoundaryEvent.class, new ActivityDrawInstruction() {
      ...
      public void draw(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode) {
       // System.out.print("流程图========x=="+bpmnModel.getGraphicInfo(flowNode.getId()).getX()+"========y=="+bpmnModel.getGraphicInfo(flowNode.getId()).getY());
  ...
  protected DefaultProcessDiagramCanvas generateProcessDiagram(BpmnModel bpmnModel, String imageType, 
    ...
    int count=highLightedActivities.size();//已完成任务的数量，增加流程图三种状态（已审核green、正在审核red、未审核black）的显示，
    for (FlowNode flowNode : bpmnModel.getProcesses().get(0).findFlowElementsOfType(FlowNode.class)) {
      //System.out.println("1111my task!!!!!!!!!!!!!!!flowNode.getId()======"+flowNode.getId()+"===highLightedActivities.size()===="+highLightedActivities.size()+"====count==="+count);
      count=count-1;
      drawActivity(processDiagramCanvas, bpmnModel, flowNode, highLightedActivities, highLightedFlows, scaleFactor,count);
    }
////以下又重复执行一遍。不知为什么？不要好像也可以！
    for (Process process: bpmnModel.getProcesses()) {
      count=highLightedActivities.size();//已完成任务的数量
      for (FlowNode flowNode : process.findFlowElementsOfType(FlowNode.class)) {
        //System.out.println("22222my task!!!!!!!!!!!!!!!flowNode.getId()======"+flowNode.getId()+"===highLightedActivities.size()===="+highLightedActivities.size()+"====count==="+count);
        count=count-1;
        drawActivity(processDiagramCanvas, bpmnModel, flowNode, highLightedActivities, highLightedFlows, scaleFactor,count);
      }
    }
  ...
  protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, 
      FlowNode flowNode, List<String> highLightedActivities, List<String> highLightedFlows, double scaleFactor,int count) {
    ...
    if (drawInstruction != null) {
      ...
      if (highLightedActivities.contains(flowNode.getId())) {
        drawHighLight(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()),count);
      }
      ...
      for (FlowElement nestedFlowElement : ((FlowElementsContainer) flowNode).getFlowElements()) {
        if (nestedFlowElement instanceof FlowNode) {
          drawActivity(processDiagramCanvas, bpmnModel, (FlowNode) nestedFlowElement, 
              highLightedActivities, highLightedFlows, scaleFactor,count);
        }
      }
    }
  }
  ...
  private static void drawHighLight(DefaultProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo,int count) {
    processDiagramCanvas.drawHighLight((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight(),count);
