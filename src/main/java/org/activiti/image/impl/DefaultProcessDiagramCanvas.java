...
public class DefaultProcessDiagramCanvas {
  ...
  protected static Color HIGHLIGHT_COLOR_CURRENT =  new Color(255, 0, 0);
  //protected static Color HIGHLIGHT_COLOR_CURRENT = new Color(19, 0, 255);
  ...
  //增加了一个count参数，表示已完成任务的数量
  public void drawHighLight(int x, int y, int width, int height,int count) {
    //System.out.println("history=======count============="+count);
    Paint originalPaint = g.getPaint();
    Stroke originalStroke = g.getStroke();
   if((count==0)){
      g.setPaint(HIGHLIGHT_COLOR_CURRENT);//count=0表示倒数第一个是当前正在要处理的任务。增加了一个count参数，表示已完成任务的数量
    }else{
      g.setPaint(HIGHLIGHT_COLOR);
    }
    g.setStroke(THICK_TASK_BORDER_STROKE);
