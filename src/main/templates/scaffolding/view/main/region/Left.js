 ...
 * 左边的菜单区域，放树形菜单
 */
Ext.define('appMis.view.main.region.${className}Left', {
    extend: 'Ext.panel.Panel',
    xtype: '${propertyName}mainleft',
    id: '${propertyName}mainleft',
   requires: [
        'appMis.view.main.menu.${className}TreeMainMenu'
    ],
    scrollable: true,
    //width: 200,
    width:'20%',
            ...
            listeners: {
                click: function (combo) {
                    Ext.Ajax.request({//根据用户角色的权限用来确定界面上的内容哪些隐藏或不隐藏,生成操作树。
                        ...
                        success://回调函数
                            function (resp, opts) {//成功后的回调方法
                              ...
                              //  Ext.getCmp("${propertyName}mainroot").getViewModel().set('systemButtonMenu',eval('(' + obj1.roots + ')'));//当前角色存入ViewModel
                                Ext.getCmp("${propertyName}mainroot").getViewModel().set('systemTreeMenu',eval('(' + obj.roots + ')'));//当前角色存入ViewModel
                             }
                    })
                   // Ext.getCmp("${propertyName}treemainmenu").setRootNode(Ext.getCmp("${propertyName}treemainmenu").up('app-${propertyName}main').getViewModel().get('systemTreeMenu'))//重新设置树状目录
                    Ext.getCmp("${propertyName}treemainmenu").setRootNode( Ext.getCmp("${propertyName}mainroot").getViewModel().get('systemTreeMenu'));//重新设置树状目录
                    Ext.getCmp("${propertyName}treemainmenu").expandPath("/p1000")//展开树状目录到path的位置
                }
            }
        },
        {
            xtype : '${propertyName}treemainmenu',
            id:'${propertyName}treemainmenu',
            listeners: {
