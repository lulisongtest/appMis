/*Wed Feb 26 00:56:23 CST 2020陆立松自动创建
*/
...
class TeacherController {
    ...
    def selectImport() throws Exception {
        ...
        String  query = "from DynamicTable where tableNameId='teacher'"
        ...
        def f = request.getFile('teacherexcelfilePath')
        ...
        try {
            ...
            while (a <= rowcount) {
                ...
                query = "FROM  Teacher where " + fieldItem1[0] + "='" + keyName+"'"
                def list8 = Teacher.findAll(query)
                if (list8.size() != 0) {
                    query = "UPDATE teacher  set " + values_update + " where " + fieldNameKey + "='" + keyName+"'"
                } else {
                    query = "insert into teacher " + columnsss + " values (" + values + ")";
                }
    ...
    def selectExport() throws Exception {
        ...
        // println("query==="+query)
        def list = Teacher.findAll(query)
    ...
    def read() {
        ...
        def list = Teacher.createCriteria().list(params) {
        ...
        render(contentType: "text/json") {
            teachers  list.collect() {
                [
                        ...
                        workDate: it?.workDate ? new java.text.SimpleDateFormat("yyyy-MM-dd").format(it?.workDate) : "",
                        treeId: it.treeId,
    ...
    def save() {
        ...
        // println("saveTeacher=== params.dep_tree_id=========="+ params.dep_tree_id)
        String dep_tree_id = params.dep_tree_id ? params.dep_tree_id : "";
        String query = "from Teacher where username='" + params.username + "'"
        //println("query==="+query)
        def dataListTeacher = Teacher.findAll(query)
        // println("dataListTeacher.size()==="+dataListTeacher.size())
        if (dataListTeacher.size() > 0) {
            // println("保存过")
            params.truename= params.truename?params.truename:"未知"
            teacherInstance = Teacher.findById(dataListTeacher[0].getId())
            teacherInstance.setProperties(params)
        } else {
            // println("没有保存过")
        ...
        try {
            ...
            Teacher i
            i = teacherInstance.save()//返回当前保存的记录
            if (i != null) {
    ...
    @Transactional
    def update() {
        Teacher teacherInstance = Teacher.findById(params.id)
        if (teacherInstance.getIdNumber()&&(teacherInstance.getIdNumber().length()==18)&&(teacherInstance.getIdNumber().toString() != params.idNumber) && (teacherInstance.getBirthDate().toString() == params.birthDate)) {//修改了工资制度
        ...
        //println("-------------teacherInstance.currentStatus==============="+teacherInstance.currentStatus)
        try {
            Teacher i
            i = teacherInstance.save(flush: true)//返回当前保存的记录
            if (i != null) {
                // response.getWriter().write("success0");//也可以
                render "success"
                return
    ...
    def delete() {
        ...
        try {
            Teacher i
            i = teacherInstance.delete(flush: true)//返回当前保存的记录
            render "success"//删除成功
            return
        } catch (e) {
            render "failure"//删除失败
    ...
    def deleteAll() {
        ...
        /*//可以删除分类
        if (dep_tree_id == '1000') {
            query = "FROM  Teacher"; //全部人员信息
            teacherquery = "Delete FROM  teacher"//删除所有人员信息
        } else {
            if (dep_tree_id.length() <= 6) {
                query = query + " where substring(treeId, 1,LENGTH('" + dep_tree_id + "'))='" + dep_tree_id + "'"//各区或分类全部单位人员信息
                teacherquery = "Delete FROM  teacher  where substring(tree_id, 1,LENGTH('" + dep_tree_id + "'))='" + dep_tree_id + "'"//删除各区或分类全部单位人员信息
            } else {
                query = query + " where treeId='" + dep_tree_id + "'"//指定单位人员信息
                teacherquery = "Delete FROM  teacher  where tree_id='" + dep_tree_id + "'"//删除指定单位人员信息
            }
        }*/
        if (dep_tree_id.length() <= 6) {
        ...
        String query = "from Teacher where treeId='" + dep_tree_id + "'";
        def list = Teacher.findAll(query)
        if (list.size() == 0) {
            render "failure1"//没有人员信息
            return
        }
        query = "Delete FROM  teacher where tree_id='" + dep_tree_id + "'";
        try {
            sql.execute(query)
            render "success"//删除成功
            return
    ...
    //把单张照片导入到Teacher表中的photo字段中
    @Transactional
    def importPhoto() {
        // println("导入Teacher照片importPhoto======params.teacherCode=========" + params.teacherCode);
        String query = "from Teacher where teacherCode='" + params.teacherCode + "'"// println("query====="+query)
        Teacher teacherInstance = Teacher.findAll(query)[0]
        if (!teacherInstance) {
            teacherInstance = new Teacher()
        }
        def f = request.getFile('teacherPhoto')//f.size上传文件的大小，如是File f 对象则f.length()是文件的大小
        def fileName
        def filePath
        //if (!f.empty) {
        if (f && !f.empty) {
            if (f.size > 8192000) {
                render "{success:false,info:'错误！导入Teacher照片大小超过8M的限制！'}" //导入后文件被裁剪大约小于10
                return
            }
            fileName = f.getOriginalFilename()
            ...
            String originfile = filePath + params.teacherCode + "photo1." + fileType
            String imagefile = filePath + params.teacherCode + "photo2." + fileType
            ...
            teacherInstance.setPhoto(new javax.sql.rowset.serial.SerialBlob(data))
            teacherInstance.save(flush: true)
    ...
    //Add窗口中把单张照片导入到Teacher表中的photo字段中
    @Transactional
    def importPhotoAdd() {
        //println("导入Teacher照片importPhoto======params.teacherCode=========" + params.teacherCode);
        String query = "from Teacher where username='" + params.username + "'"// println("query====="+query)
        //println("importPhotoAdd===query==="+query)
        Teacher teacherInstance = Teacher.findAll(query)[0]
        if (!teacherInstance) {
            teacherInstance = new Teacher(params)
        }
        def f = request.getFile('teacherPhoto1')//f.size上传文件的大小，如是File f 对象则f.length()是文件的大小
        def fileName
        def filePath
        //if (!f.empty) {
        if (f && !f.empty) {
            ...
            teacherInstance.setPhoto(new javax.sql.rowset.serial.SerialBlob(data))
            String truename= teacherInstance.getTruename()
            teacherInstance.setTruename(truename?truename:"未知")
            teacherInstance.save(flush: true)
            File filep = new File(originfile);//删除临时文件
            filep.delete();
            filep = new File(imagefile);//删除临时文件
    ...
    //Edit窗口中把单张照片导入到Teacher表中的photo字段中
    @Transactional
    def importPhotoEdit() {
        //println("导入Teacher照片importPhoto======params.teacherCode=========" + params.teacherCode);
        String query = "from Teacher where username='" + params.username + "'"// println("query====="+query)
    ...
    //批量上传照片文件，然后导入Teacher表中，【调试阶段暂时不删除批量上传的照片文件】
    @Transactional
    def uploadFile() {
        ...
        try {
            ...
            //String imagefile=uploadPath + params.teacherCode + "photo2.jpg"
            File file = new File(originfile)//file.length()是文件的大小
            f.transferTo(file)//上传的文件先存入临时文件//f.size上传文件的大小
            String str = params.name
            String teacherCode = "", temp = "";
            for (int i = 0; i < str.length(); i++) {
                if (str.charAt(i) >= 48 && str.charAt(i) <= 57) {
                    // teacherCode+=str.charAt(i);
                    teacherCode = (str.substring(i)).split("\\.")[0];//取照片名字后面的身份证号码
                    break;
                }
            }
            String query = "from Teacher where teacherCode='" + teacherCode + "'"// println("query====="+query)
            Teacher teacherInstance = Teacher.findAll(query)[0]
            if (!teacherInstance) {
                return "success";//有照片，没有Teacher信息
            }
            ...
            output.close();
            input.close();
            teacherInstance.setPhoto(new javax.sql.rowset.serial.SerialBlob(data))//照片导入Teacher表中
            teacherInstance.save(flush: true)
    ...
    def displayPhotoAdd() {
        ...
        String imagefile = filePath + params.teacherCode + "photo2.jpg"
        ...
        BufferedImage teacherImage;
        String fileType

        // if (teacherInstance&&teacherInstance.getPhoto()) {
        try {
            //println("有照片")
            //inputStream =new InputStream(imagefile)
            //从数据库中读出JPG文件以JPG文件显示
            outputStream = response.getOutputStream();
            teacherImage = ImageIO.read(new FileImageInputStream(new File(imagefile)));
            ImageIO.write(teacherImage, "JPEG", response.getOutputStream());
            outputStream.flush()
            outputStream.close();
            inputStream.close();
        } catch (Exception e) {
            ...
            //还没有上传文件
            //默认显示一个文件（未知名.png）
            // if(teacherInstance&&teacherInstance.getSex()=="男"){
    ...
    def displayPhoto() {
        ...
        //println("从数据库调出职工的照片照片displayTeacherImage======params.teacherCode=========" + params.teacherCode);
        // println("登录用户username========" + username);
        // println("登录用户usernamechineseAuthority========" + chineseAuthority);
        String query
        //是管理者或是自己的项目,如已上传直接显示，否则必须是完成审核通过的文件才能显示。
        query = "from Teacher where username='" + params.username + "'"
        ...
        try {
            //println("有照片")
            Blob blob = teacherInstance.getPhoto();
            inputStream = blob.getBinaryStream();
            //从数据库中读出JPG文件以JPG文件显示
            outputStream = response.getOutputStream();
            teacherImage = ImageIO.read(inputStream);
            ImageIO.write(teacherImage, "JPEG", response.getOutputStream());
            outputStream.flush()
            outputStream.close();
            inputStream.close();
        } catch (Exception e) {
            // println("无照片sex====="+teacherInstance.getSex())
            //还没有上传文件
            //默认显示一个文件（未知名.png）
            if (teacherInstance && teacherInstance.getSex() == "男") {
                inputStream = new FileInputStream(new File(getServletContext().getRealPath("/") + "photoD\\" + "male.gif"));
...
//----------------------------------------------------------------------------------------------------
    ...
    def json(list){
        ...
        //从定义的类中取出有效字段数n
        def list1=Teacher.getDeclaredFields()
        int n=0
