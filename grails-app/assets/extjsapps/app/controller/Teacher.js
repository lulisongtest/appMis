...
Ext.define('appMis.controller.Teacher', {
    extend: 'Ext.app.Controller',
    init: function () {
        this.control({
            'selectExportTeacherItem   button[action=exportToExcel]':{//选择导出信息项目
                click: this.exportExcel
            },
            'selectImportTeacherItem   button[action=importFromExcel]':{//选择导入信息项目
                click: this.importExcel
            },
            'teacherviewport >  toolbar > button[action=modify]': {
                click: this.modify
            },
           'teacheredit  button[action=save]': {
                click: this.update
            },
            'teacheredit  button[action=importPhotoEdit]': {
                click: this.importPhotoEdit//在编辑窗口中导入个人照片
            },
            'teacheradd  button[action=save]': {
                click: this.save
            },
            'teacheradd  button[action=importPhotoAdd]': {
                click: this.importPhotoAdd//在新增窗口中导入个人照片
            },
            'teacherviewport > toolbar >  button[action=add]': {
                click: this.add
            },
            'teacherviewport >  toolbar > button[action=addRow]': {
                click: this.addRow
            },
            'teacherviewport > toolbar >  button[action=delete]': {
                click: this.delete
            },
            'teacherviewport > toolbar >  button[action=deleteAll]': {
                click: this.deleteAll//删除整个单位的所有学生
            },
            //------------------------------------------
            'teacherviewport > toolbar >  button[action=print]': {
                click: this.print
            },
           'teacherDisplayDetail  button[action=print]': {
    ...
    importExcel: function (button) {
       // alert("导入EXCEL,从Excel表导入到数据库.................")
        var fieldItem=Ext.getCmp('itemselectorImportTeacher').getValue();
        win_select=Ext.getCmp('selectImportTeacherItem');
        form = Ext.getCmp('selectImportTeacherItemForm');
        if (form.form.isValid()) {
            ...
            form.getForm().submit({
                url:'/appMis/teacher/selectImport',
                method: "POST",
                params : {
                ...
                success: function (fp, o) {
                    ...
                    setTimeout(function () {
                        ...
                        panelStore.proxy.api.read = '/appMis/teacher/read?dep_tree_id=' + currentTreeNode.substr(1);
    ...
    exportExcel: function (record){
        ...
        fieldItem=Ext.getCmp('itemselectorExportTeacher').getValue();//所选择要导出的字段
        var p="";
        for(var i=0;i<fieldItem.length;i++){
            p=p+'&p='+fieldItem[i]//所选择要导出的字段重新组成url中传递的数组参数
        }
        var s="/appMis/teacher/selectExport?dep_tree_id="+currentTreeNode.substr(1)+p;
    ...
    modify: function () {
        var record = (Ext.getCmp("teachergrid")).getSelectionModel().getSelection();
        if (record.length != 1) {
            Ext.MessageBox.show({
                title: "提示",
        ...
        var view = Ext.widget('teacheredit');//编辑窗口中的记录与Grid中选中的记录是同一条记录
        view.down('form').loadRecord(record[0]);
        selectUsername=record[0].get("username");//选中的用户名
        Ext.getCmp("photoEdit").setSrc('/appMis/teacher/displayPhoto?username=' + selectUsername+'&time=' + new Date());//显示用户的照片
    },
   ...
   //在编辑窗口中编辑记录后进行保存更新
    update: function (button) {
        ...
        form = win.down('form');
        record = ((Ext.getCmp("teachergrid")).getSelectionModel().getSelection())[0];
        values = form.getValues();
        values.birthDate = (values.birthDate).replace(/(年)|(月)/g, "-");
        values.birthDate = (values.birthDate).replace(/(日)/g, "");
        values.workDate = (values.workDate).replace(/(年)|(月)/g, "-");
        values.workDate = (values.workDate).replace(/(日)/g, "");
        record.set(values);
        win.close();
    },
    ...
    save: function (button) {
        ...
        values.birthDate = (values.birthDate).replace(/(日)/g, "");
        values.workDate = (values.workDate).replace(/(年)|(月)/g, "-");
        values.workDate = (values.workDate).replace(/(日)/g, "");        record.set(values);//因为是新增记录,不能激活Store的update:function(store,record)
    ...
    insert: function (record, win) {
        var panelStore = (Ext.getCmp("teachergrid")).getStore();
        Ext.Ajax.request({
            url: '/appMis/teacher/save?dep_tree_id=' + currentTreeNode.substr(1),
            params: {
    ...
    add: function () {
        ...
        var record = Ext.create('appMis.model.TeacherModel', {
                            ...
                            race:"",
                            politicalStatus:"",
                            workDate:(new Date()).pattern("yyyy-MM-dd"),
                            treeId:currentTreeNode.substr(1),
                            currentStatus:"",
        });
        //设置初值
        var view = Ext.widget('teacheradd');
    ...
    addRow: function () {
        ...
        record = Ext.create('appMis.model.TeacherModel', {
                    username:"klmya"+Math.floor(Math.random()*10000),
                    truename:"",
                    email:"",
                    department:departmentName,
    ...
    remove: function (store, record) {
        Ext.Ajax.request({
            url: '/appMis/teacher/delete?id=' + record.get("id"),
            method: 'POST',
    ...
    deleteAll: function (button) {
        ...
        var panelStore = (Ext.getCmp('teachergrid')).getStore();
        var myMask = new Ext.LoadMask(Ext.getCmp('teachergrid'), {msg: "正在删除所有单位人员信息，请等待..."});
        myMask.show();
        Ext.Ajax.timeout = 0;
        Ext.Ajax.request({
            url: '/appMis/teacher/deleteAll?dep_tree_id=' + currentTreeNode.substr(1)+'&departmentName='+departmentName,
            method: 'POST',
            async: false,
            success://回调函数
                function (resp, opts) {//成功后的回调方法
    ...
    importPhotoAdd: function (button) {
        ...
        form = Ext.getCmp('teacherPhotoForm1');
        if (form.form.isValid()) {
            if (Ext.getCmp('teacherPhoto1').getValue() == '') {
                Ext.Msg.alert('系统提示', '请选择你要上传照片的文件');
                return;
            }
            form.getForm().submit({
                // url: '/appMis/teacher/importPhotoAdd?dep_tree_id=' + currentTreeNode.substr(1),
                url: '/appMis/teacher/importPhotoAdd',
                method: "POST",
                async: false,
                params: {
                         username:record.get("username"),
                ...
                success: function (fp, o) {
                    if (o.result.success) {
                        ...
                        setTimeout(function () {
                            Ext.Msg.hide();
                            //Ext.getCmp("photoAdd").getEl().dom.src = '/appMis/teacher/displayPhoto?username=' + selectUsername + '&time=' + new Date()//显示用户的照片，也可以
                            Ext.getCmp("photoAdd").setSrc('/appMis/teacher/displayPhoto?username=' + selectUsername + '&time=' + new Date())//更好，显示用户的照片
                                (Ext.getCmp("teachergrid")).getStore().loadPage(1);//定位grid到第一页
    ...
    importPhotoEdit: function (button) {
        //alert("importPhoto!!!!==="+Ext.getCmp('teacherPhoto1').getValue())
        form = Ext.getCmp('teacherPhotoForm1');
        if (form.form.isValid()) {
            if (Ext.getCmp('teacherPhoto1').getValue() == '') {
                Ext.Msg.alert('系统提示', '请选择你要上传照片的文件');
            ...
            form.getForm().submit({
                url: '/appMis/teacher/importPhotoEdit?username=' + selectUsername,
                method: "POST",
                async: false,
                waitMsg: '正在导入个人照片的文件，请等待...',
                waitTitle: '请等待',
                success: function (fp, o) {
                    if (o.result.success) {
                        ...
                        setTimeout(function () {
                            Ext.Msg.hide();
                            //Ext.getCmp("photoEdit").getEl().dom.src = '/appMis/teacher/displayPhoto?username=' + selectUsername + '&time=' + new Date()//也可以
                            Ext.getCmp("photoEdit").setSrc('/appMis/teacher/displayPhoto?username=' + selectUsername+'&time=' + new Date());//更好，显示用户的照片
                        }, 2500)
                    } else {
                        Ext.Msg.show({
                            title: '操作提示 ',
 ...
 //----------------------------------------------------------------------------------------
    ...
    printDisplayDetaill: function (button) {
        var LODOP = getLodop();
        LODOP.PRINT_INIT();
        LODOP.ADD_PRINT_HTM('1%', '1%', '98%', '98%', document.getElementById("teacherDisplayDetail").innerHTML);
    ...
    print: function (button) {
        Ext.Ajax.request({
            url: '/appMis/teacher/printTeacher?dep_tree_id=' + currentTreeNode.substr(1),
            method: 'POST',
            success://回调函数
                function (resp, opts) {
                    {
                        ...
                        tpl1.append('tpl-1', data1.teachers);
                        var LODOP = getLodop();
