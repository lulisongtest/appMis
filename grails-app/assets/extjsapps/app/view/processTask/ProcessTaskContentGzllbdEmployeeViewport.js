...
var newvalue1
var newvalue2
Ext.define('appMis.view.processTask.ProcessTaskContentGzllbdEmployeeViewport', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.processTaskContentGzllbdEmployeeViewport',
    defaults: {
        autoScroll: true,
        bodyPadding: 0,
        border: 0,
    ...
    items: [
        {
            ...
            items: [
                ...
                },
                {
                    xtype: "button",
                    columnWidth: 0.18,
                    text: "查看职务与岗位变动前人员个人详细信息",
                    handler : function(){
                        //alert("procTaskContentVar====="+procTaskContentVar.zbr)
                       //从form中获取数据
                        var values=Ext.getCmp('gzllbdEmployeeform').getValues()
                ...
                },
                {
                    ...
                    handler : function(){
                        ...
                        Ext.create('Ext.window.Window',{
                            id: 'employeeGzllbdSalaryDetailWin',
                            width: '100%',
                            height: '100%',
                            title: departmentDetailTitle+"__"+('显示当前职工【' + values.employeeCode + ' ' + values.name + (new Date(values.sbDate).pattern("yyyy年MM月"))+ ' 】的职务与岗位变动详细信息'),
                            autoShow: true,
                            modal: true,//创建模态窗口
                            layout:{
                                type: 'vbox',
                            ...
                            items :[
                                {
                                    ...
                                    items: [{
                                        xtype: 'component',
                                        id: "displayEmployeeGzllbdsbDetail",
                                        autoEl: {
                                            tag:'iframe',
                                            style:'height:100%;width:100%;border:none',
                                            src:'/appMis/employeeGzll/displayGzllbdsb?taskId='+taskId+'&procDefId=' + procDefId + '&executionId=' + executionId + '&procInstId=' + procInstId+ '&employeeCode=' + values.employeeCode+'&name='+values.name+'&departmentName='+values.department+'&zbr='+values.zbr+'&shra='+values.shra+'&shrb='+values.shrb+'&shrc='+values.shrc+'&shrd='+values.shrd+'&currentSbDate='+(new Date(values.sbDate).pattern("yyyy-MM-dd"))
                                            //src: 'http://' + window.location.host + '/kzykyMis/static/photoD/xxx.pdf'
                                        }
                                    }]
                               }],
                            buttons:[
                                {
                                    ...
                                    handler: function (button, e) {
                                        ...
                                        Ext.Msg.show({
                                            title: '操作提示 ',
                                           // msg: "<a href=\"http://" + window.location.host + "/appMis/static/appTmp/" +(new Date(values.sbDate).pattern("yyyy年MM月"))+ values.employeeCode+ values.name +"个人退休工资详细信息.xlsx\"><b>保存该EXCEL文件</b></a>",
                                            msg: "<a href=\"http://" + window.location.host + "/appMis/static/appTmp/" + values.department+values.name+(new Date(values.sbDate).pattern("yyyy年M月"))+ "个人职务与岗位变动申报表.xlsx\"><b>保存该EXCEL文件</b></a>",
                                ...
                                },
                                {
                                    ...
                                    handler: function (button, e) {
                                        Ext.Ajax.request({
                                            url: '/appMis/employeeGzll/deleteExcelGzllbdsb?departmentName='+values.department+'&employeeCode=' + values.employeeCode+'&name='+ values.name+'&currentSbDate='+(new Date(values.sbDate).pattern("yyyy-MM-dd")),
                                            ansync: false,
                                            method: 'POST',
                                            success://回调函数
                                                function (resp, opts) {
                                        ...
                                        Ext.getCmp("employeeGzllbdSalaryDetailWin").close()
                                    }
                                }],
                            listeners : {
                                'hide':function() {
                ...
                },
                {
                    xtype: "button",
                    columnWidth: 0.22,
                    id:'reImportGzllSbExcel0',
                    text: '重新导入修改后的职务与岗位变动后详细信息',
                    ...
                    handler : function(){
                        ...
                        var form = Ext.getCmp('reImportGzllSbExcel')
                        if (form.form.isValid()) {
                            if (Ext.getCmp('gzllsbexcelfilePath').getValue() == '') {
                                Ext.Msg.alert('系统提示', '请选择你要上传的文件');
                                return;
                            }
                            form.getForm().submit({
                                url: '/appMis/employeeGzll/reImportGzllExcel?taskId='+taskId+'&procDefId=' + procDefId+ '&executionId=' + executionId + '&procInstId=' + procInstId,
                ...
                },
                {
                    ...
                    items: [{
                        ...
                        // labelAlign: 'right',
                        //fieldLabel: '选择文件',
                        name: 'gzllsbexcelfilePath',
                        id: 'gzllsbexcelfilePath',
                        buttonText: '选择文件......',
